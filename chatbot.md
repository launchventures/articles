...
To explain, let's take an example of a dialog between a girl called Cherry and a cafe waiter called Happy.
...
Now let's understand the key elements in the context of this conversation.
...
An Intent is an option from the set of expected motivations, a user could communicate when interacting with someone or a chatbot. Taking the above conversation example, we can see that Cherry had the following set of intentions:
...
As you can observe, a best practice in naming Intents is to use a `verb + noun` format when applicable. The `noun` is also often an entity which we shall read about next.
...
| Intent | Utterance | Entity | Set of Values |
| --- | --- | --- | --- |
...
```english
Context 1: Select color, Select drink ware
